...
const API_URL = window.localStorage.getItem("sl_api_url") || "http://localhost:8081";
...
async function api(path, options = {}) {
  const token = window.localStorage.getItem("sl_admin_token");
  const response = await fetch(`${API_URL}/api/marketing${path}`, {
    ...options,
    headers: { "Content-Type": "application/json", ...(token ? { Authorization: `Bearer ${token}` } : {}), ...(options.headers || {}) },
  });
  const json = await response.json().catch(() => ({}));
  if (!response.ok || !json.success) throw new Error(json.message || "Could not reach Marketing Cloud");
  return json;
}
...
function downloadCreative(){
  ...
  const link=document.createElement("a");link.download="savitri-livings-creative.png";link.href=canvas.toDataURL("image/png");link.click();creativeDownloads++;$("#creative-count").textContent=creativeDownloads;
  api("/content", { method:"POST", body:JSON.stringify({ type:"creative", name: offer, format, metadata:{ theme:$("#creative-theme").value } }) }).catch(()=>{});
  toast("Creative downloaded.");
}

async function loadDashboard(){
  try {
    const { dashboard } = await api("/dashboard");
    $("#emails-sent").textContent=dashboard.emailsSent;
    $("#campaign-count").textContent=dashboard.campaigns;
    $("#creative-count").textContent=dashboard.creatives + creativeDownloads;
    $("#listing-count").textContent=dashboard.marketplaceListings;
  } catch { /* Marketing Cloud can still be previewed before an admin signs in. */ }
}

async function fetchCustomers(){
  try {
    const data=await api("/customers");
    customers=data.customers.map(customer=>({ id:customer._id, fullName:customer.fullName, email:customer.email, phone:customer.phone }));
    selected.clear(); renderCustomers(); toast(`${customers.length} Savitri and Thikana customers loaded.`);
  } catch (error) { toast("Sign in to Savitri Livings admin first, then fetch shared customers."); }
}
...
$("#fetch-customers").addEventListener("click",fetchCustomers);
$("#save-audience").addEventListener("click",async()=>{const name=$("#audience-name").value.trim();if(!name) return toast("Enter an audience name first.");try { await api("/audiences",{method:"POST",body:JSON.stringify({name,customerIds:[...selected]})}); toast(`“${name}” saved to Savitri Livings.`); } catch { toast("Sign in to Savitri Livings admin to save this audience."); }});
...
$("#save-caption").addEventListener("click",async()=>{try { await api("/content",{method:"POST",body:JSON.stringify({type:"caption",name:$("#caption-type").value,body:$("#caption").value})}); toast("Caption saved to Savitri Livings."); } catch { toast("Sign in to Savitri Livings admin to save this caption."); }});
renderCustomers();renderTemplate();loadDashboard();
