...
export function Modal({ onClose, children }) {
  return (
    <AnimatePresence>
      <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
        onClick={onClose}
        style={{ position:"fixed", top:0, left:0, right:0, bottom:0, background:"rgba(26,20,9,.65)", zIndex:200, display:"flex", alignItems:"flex-end", justifyContent:"center" }}>
        <motion.div initial={{ y:"100%" }} animate={{ y:0 }} exit={{ y:"100%" }}
          transition={{ type:"spring", damping:28, stiffness:300 }}
          onClick={e=>e.stopPropagation()}
          style={{ background:"#fff", width:"100%", maxWidth:480, borderRadius:"1.5rem 1.5rem 0 0", maxHeight:"90vh", overflowY:"auto", paddingBottom:"env(safe-area-inset-bottom,0px)" }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"1rem 1.25rem", borderBottom:"1px solid #e8dfd0", background:"#fff" }}>
            <div style={{ width:36, height:4, background:"#d8cfc0", borderRadius:2 }}/>
            <button onClick={onClose} style={{ width:28, height:28, borderRadius:"50%", border:"none", background:"#f0e6d0", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
              <X style={{ width:13, height:13, color:"#8c7258" }}/>
            </button>
          </div>
...
const WA_FALLBACK = "6207855397";

export function QRPayModal({ amount, orderId, upiId, onClose }) {
  const deepLink = `upi://pay?pa=${encodeURIComponent(upiId)}&pn=${encodeURIComponent("Savitri Livings")}&am=${amount}&cu=INR&tn=${encodeURIComponent(`Order #${orderId}`)}`;
  const copy = () => {
    navigator.clipboard.writeText(upiId);
    toast && toast.success?.("UPI ID copied!");
  };
  ...
  return (
    <Modal onClose={onClose}>
      <div style={{ padding:"1.25rem", display:"flex", flexDirection:"column", gap:"1rem" }}>
        ...
        <a href={`https://wa.me/91${import.meta.env.VITE_WHATSAPP || WA_FALLBACK}?text=${encodeURIComponent(`Payment done for order #${orderId}. Amount: Rs.${amount}`)}`}
          target="_blank" rel="noopener noreferrer" className="btn-wa" style={{ justifyContent:"center" }}>
          Confirm Payment on WhatsApp
        </a>
        <button onClick={onClose} className="btn-ghost" style={{ justifyContent:"center", fontSize:13 }}>Close</button>
