    ...
    <motion.div layout initial={{ opacity:0, scale:.97 }} animate={{ opacity:1, scale:1 }} exit={{ opacity:0 }} className="card" style={{ overflow:"hidden", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ padding:"0.875rem", flex:1, display:"flex", flexDirection:"column" }}>
        ...
        <Link to={`/product/${p._id}`} style={{ textDecoration:"none", flex:1 }}>
          <h3 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"1rem", color:"#3a2c1a", lineHeight:1.2, overflow:"hidden", textOverflow:"ellipsis", display:"-webkit-box", WebkitLineClamp:2, WebkitBoxOrient:"vertical" }}>{p.name}</h3>
        </Link>
      ...
      <div style={{ background:"linear-gradient(135deg,#765027,#4d3219)", padding:"2.5rem 1rem 3rem" }}>
        <div style={{ maxWidth:1280, margin:"0 auto" }}>
          ...
          <div style={{ position:"relative", maxWidth:500 }}>
            ...
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search rings, necklaces, earrings…"
        ...
        <div className="no-scrollbar" style={{ display:"flex", gap:8, overflowX:"auto", padding:"1rem 0", margin:"0 -1rem", paddingLeft:"1rem", paddingRight:"1rem" }}>
          {CATS.map(c => (
            <button key={c} onClick={() => setCat(c)}
              style={{ flexShrink:0, padding:"0 1.25rem", height:36, borderRadius:"9999px", fontSize:"0.8rem", fontWeight:500, cursor:"pointer", border: cat===c ? "none" : "1px solid #d8cfc0", background: cat===c ? "#765027" : "white", color: cat===c ? "white" : "#5c4a32", fontFamily:"'DM Sans',sans-serif" }}>
